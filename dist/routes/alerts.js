...
import { resolveAlertSchema, triggerAlertSchema, } from "../validators/schemas.js";
...
// POST /alerts/resolve
alertRouter.post("/resolve", async (req, res) => {
    try {
    ...
    try {
        ...
        if (existingActiveAlert) {
            return res.status(200).json({
                ...
                usedLocation: existingActiveAlert.latAtTrigger != null &&
        ...
        const finalLatitude = latitude !== undefined
            ? latitude
            : user.lastLat !== null && user.lastLat !== undefined
                ? user.lastLat
                : undefined;
        const finalLongitude = longitude !== undefined
            ? longitude
        ...
        try {
            ...
            await alertQueue.add("sendEmergencyAlert", {
                alertId: alert.id,
                userId,
...
alertRouter.get("/active/:userId", async (req, res) => {
    try {
...
alertRouter.get("/history/:userId", async (req, res) => {
    try {
        const userId = String(req.params.userId);
