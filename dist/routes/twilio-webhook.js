...
async function simulate911Escalation(alertId) {
    ...
    try {
        ...
        if (CallStatus === "completed") {
            ...
            if (latestAction?.outcome === "in-progress") {
                ...
                console.log(`✅ Call succeeded for alert ${action.alertId}, but alert stays ACTIVE until user clicks I'm safe`);
                return res.sendStatus(200);
            }
            await db.alertAction.update({
                where: { id: action.id },
            ...
            if (attemptCount < MAX_CALL_ATTEMPTS) {
                console.log(`🔁 Scheduling delayed retry for alert ${action.alertId}. Attempt ${attemptCount + 1}/${MAX_CALL_ATTEMPTS} in 30s`);
                await alertQueue.add("retryEmergencyCall", { alertId: action.alertId }, {
                ...
                console.log(`🚨 Max call attempts reached for alert ${action.alertId}. Simulating 911 escalation`);
