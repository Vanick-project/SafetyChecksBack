...
export async function scheduleCheckIn(userId) {
    ...
    await checkInQueue.add("send-check-in", { userId, attempt: 1 }, {
...
export async function rescheduleAfterOk(userId) {
    const mainJob = await checkInQueue.getJob(`checkin-${userId}`);
    await mainJob?.remove();
    const reminderJob = await checkInQueue.getJob(`checkin-reminder-${userId}`);
    await reminderJob?.remove();
...
async function triggerAutomaticSOS(userId, checkInId) {
    ...
    if (user.emergencyContact) {
        try {
            ...
            console.error("❌ Automatic SOS SMS failed:", err instanceof Error ? err.message : err);
        }
        try {
            await callEmergencyContact(alert.id);
        }
        catch (err) {
            console.error("❌ Automatic SOS call failed:", err instanceof Error ? err.message : err);
        }
    }
    await scheduleCheckIn(userId);
}
new Worker("check-in", async (job) => {
    if (job.name === "send-check-in") {
        ...
        console.log(`🔔 Check-in notification sent to user ${userId}, attempt ${attempt}`);
        if (attempt < MAX_REMINDERS) {
            const reminderJob = await checkInQueue.getJob(`checkin-reminder-${userId}`);
            await reminderJob?.remove();
            await checkInQueue.add("send-check-in", { userId, attempt: attempt + 1 }, {
                jobId: `checkin-reminder-${userId}`,
                delay: TEN_MINUTES,
                removeOnComplete: true,
            });
            console.log(`⏳ Reminder ${attempt + 1} scheduled in 10 min for user ${userId}`);
            return;
        }
        await checkInQueue.add("auto-sos", {
            userId,
...
export async function handleUserResponse(userId, checkInId, response) {
    const mainJob = await checkInQueue.getJob(`checkin-${userId}`);
    await mainJob?.remove();
    const reminderJob = await checkInQueue.getJob(`checkin-reminder-${userId}`);
    await reminderJob?.remove();
    ...
    if (response === "SOS") {
        ...
        if (user.emergencyContact) {
            try {
                ...
                console.error("❌ Manual SOS SMS failed:", err instanceof Error ? err.message : err);
            }
            try {
                await callEmergencyContact(alert.id);
            }
            catch (err) {
                console.error("❌ Manual SOS call failed:", err instanceof Error ? err.message : err);
            }
        }
        await scheduleCheckIn(userId);
    }
}
//# sourceMappingURL=checkin-scheduler.js.map